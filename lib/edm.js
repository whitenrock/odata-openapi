...
module.exports.EDM = class {
   ...
   */
  get resources() {
    const resources = Object.entries(this.entityContainer ?? {}).filter(
      ([name]) => isIdentifier(name),
    );
    resources.forEach(([name, object]) => {
      const self = "SAP__self";
      if (object["$Type"]) {
        const type = object["$Type"].slice(self.length + 1);
        const element = this.#elements[`${this.namespace[self]}.${type}`];
        if (element) {
